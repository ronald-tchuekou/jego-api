...
export async function createFakeAppointments() {
    ...
    try {
        ...
        const companies = await Company.query().whereNull('blockedAt');
        ...
        for (const user of regularUsers) {
            ...
            for (let i = 0; i < appointmentCount; i++) {
                ...
                const appointmentDate = DateTime.fromJSDate(faker.date.between({ from: startDate.toJSDate(), to: endDate.toJSDate() }));
                let status;
                ...
                try {
                    ...
                    console.log(`  ✅ Created appointment: "${appointmentData.subject}" with ${company.name} on ${dateStr} at ${appointmentData.time} (${appointmentData.status})`);
                }
                catch (error) {
                    console.log(`  ❌ Failed to create appointment for ${user.displayName} with ${company.name}: ${error.message}`);
                }
            }
...
function generateAppointmentContent(companyName) {
...
export async function getAppointmentStatistics() {
    ...
    try {
        ...
        userStats.forEach((stat, index) => {
            console.log(`  ${index + 1}. ${stat.user.firstName} ${stat.user.lastName}: ${stat.$extras.count}`);
        });
...
//# sourceMappingURL=create_fake_appointments.js.map
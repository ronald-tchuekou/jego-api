...
export async function createFakeJobs() {
    ...
    try {
        ...
        if (eligibleUsers.length === 0) {
            console.log('⚠️ No eligible users found in database. Please create admin or company users first.');
            return;
        }
        ...
        const getJobDescription = (title, company) => {
            ...
            for (let i = 0; i < jobCount; i++) {
                ...
                const companyData = user.company
                    ...
                    : {
                        ...
                        companyLogo: Math.random() > 0.7 ? faker.image.url({ width: 200, height: 200 }) : null,
                ...
                const expiresAt = Math.random() > 0.3
                ...
                try {
                    ...
                    const expiryInfo = jobData.expiresAt
                        ...
                        : 'permanent';
                    console.log(`  ✅ Created job: "${jobData.title}" at ${jobData.companyName} (${jobData.status}, ${expiryInfo})`);
                }
                catch (error) {
                    console.log(`  ❌ Failed to create job "${jobData.title}" for ${userName}: ${error.message}`);
                }
            }
...
//# sourceMappingURL=create_fake_jobs.js.map